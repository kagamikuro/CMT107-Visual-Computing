...
public class Rugby extends SObject {
	...
	public Rugby(float radius, int slices, int stacks){
		super();
		this.radius = radius;
		this.slices = slices;
		this.stacks = stacks;
	...
	protected void genData() {
		...
		for (i = 1; i < stacks; i++) {
			for (j = 0; j <= slices; j++) {
				normals[3 * k] = sin(deltaLat * i) * cos(deltaLong * j + 1);
