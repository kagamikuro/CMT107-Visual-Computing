 ...
 * First Basic.Transform according to their order in the program. This is opposite
...
public class Transform {
	...
	public void rotateA(float angle, float x, float y, float z){
		...
		Vec3float v = new Vec3float(x,y,z); //for normalising x, y, z
	...
	public void lookAt(float ex,float ey, float ez, 
		...
		Vec3float e = new Vec3float(ex,ey,ez); //eye position
		Vec3float a = new Vec3float(ax,ay,az); //look at position
		Vec3float u = new Vec3float(ux,uy,uz); // up direction
		
		Vec3float b = e.minus(a); // back direction
		Vec3float r = u.cross(b); //right direction
